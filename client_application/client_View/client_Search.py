...
class client_search(GridLayout):#innherit class GridLayout
    ...
    def search_button_pressed(self, instance):
        ...
        for result in results:
            ...
            print(result['_id'])
            sql.execute(f"select distinct k.AMOUNT, k.CASHFLOW_ID, s.TAX, k.DATE_TIME, k.TAX_PAYED, k.ID from keep_in_book k, source_of_cashflow s where k.CASHFLOW_ID = s.CASHFLOW_ID AND k.ID = '{str(result['_id'])}'")
